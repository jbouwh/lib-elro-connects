...
async def test_construct_message_generates_correct_message(hub):
    ...
    assert (
        message == '{"msgId":1,"action":"appSend","params":{"devTid":"ST_aaaaaaaaaaaa",'
...
async def test_get_device_names_sends_the_right_command(hub):
    ...
    hub.sock.sendto.assert_awaited_with(
        b'{"msgId":1,"action":"appSend","params":{"devTid":"ST_aaaaaaaaaaaa",'
...
async def test_does_not_crash_on_a_name_reply_on_unregistered_devices(hub):
    data = {
        "data": {
            "cmdId": Command.DEVICE_NAME_REPLY.value,
